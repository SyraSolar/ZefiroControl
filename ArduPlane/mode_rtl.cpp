...
void ModeRTL::update()
{
    ...
    if (_last_update_ms != 0) {
        dt = (now - _last_update_ms) * 0.001f; // Convert ms to seconds
    }
    _last_update_ms = now;
    ...
    double Mag_attitude = (eroll/tol_roll)*(eroll/tol_roll) + (epitch/tol_pitch)*(epitch/tol_pitch) + (eyaw/tol_yaw)*(eyaw/tol_yaw) +
                         (eP/tol_P)*(eP/tol_P) + (eQ/tol_Q)*(eQ/tol_Q) + (eR/tol_R)*(eR/tol_R);

    // double Mag_position = (eX/tol_X)*(eX/tol_X) + (eY/tol_Y)*(eY/tol_Y) + 
    //                       (eU/tol_U)*(eU/tol_U) + (eV/tol_V)*(eV/tol_V);

    // double Mag_altitude = (eZ/tol_Z)*(eZ/tol_Z) + (eW/tol_W)*(eW/tol_W);
    ...
    if (Mag_attitude < 1.0){
        ...
        if (Att_counter > Att_dwell) {
            for (int i = 0; i < 12; i++) {
                if (Att_flag == 0) {
                    Residual_Att[i] = zefiroControl->U_int_att[i] + zefiroControl->U_prop_att[i];
                    Att_flag = 1;
                }
                //zefiroControl->set_accumulative_error();
                zefiroControl->U_int_att[i] = 0.0;
                zefiroControl->U_prop_att[i] = 0.0;
                zefiroControl->U_eq[i] = 0.0;
            }
        } 
    } else {
        Att_counter = 0.0;
    ...
    for (int i = 0; i < 12; i++) {
        zefiroControl->U[i] = zefiroControl->U_prop_att[i] + 
        zefiroControl->U_int_att[i] + zefiroControl->U_prop_pos[i] + 
        zefiroControl->U_int_pos[i] + zefiroControl->U_prop_alt[i] + 
        zefiroControl->U_int_alt[i] + zefiroControl->U_eq[i];
    }
