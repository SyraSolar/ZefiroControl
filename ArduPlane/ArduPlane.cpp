...
const AP_Scheduler::Task Plane::scheduler_tasks[] = {
    ...
    SCHED_TASK_CLASS(AP_WindVane, &plane.g2.windvane, update, 20,  100,  79),
...
void Plane::update_logging10(void)
{
    ...
    if(plane.g2.enable_wind_log == 1) {
        //GCS_SEND_TEXT(MAV_SEVERITY_INFO, "is enabled: %d", plane.g2.windvane.enabled());
        //GCS_SEND_TEXT(MAV_SEVERITY_INFO, "-----");
        float azimuth = (ahrs.yaw_sensor*0.01f); //get_yaw();
        float raw_direction = wrap_360(degrees(plane.g2.windvane.get_raw_apparent_wind_direction_rad()));
        float direction = wrap_360(degrees(plane.g2.windvane.get_apparent_wind_direction_rad()));
        float raw_speed = plane.g2.windvane.get_raw_apparent_wind_speed();
        float speed = plane.g2.windvane.get_apparent_wind_speed();
        
        float raw_true_direction = wrap_360(degrees(plane.g2.windvane.get_raw_true_wind_direction_rad()));
        float true_direction = wrap_360(degrees(plane.g2.windvane.get_true_wind_direction_rad()));
        float raw_true_speed = plane.g2.windvane.get_raw_true_wind_speed();
        float true_speed = plane.g2.windvane.get_true_wind_speed();
        
        //ZEF
        //GCS_SEND_TEXT(MAV_SEVERITY_INFO, "1 >>> wind dir: %f --- wind speed: %f", direction, speed);
        //float corr_x=0.96;
        //float corr_y=0.60;
        //float vx_cor= corr_x*speed*cosf(radians(direction));
        //float vy_cor= corr_y*speed*sinf(radians(direction));
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "az: %.0f", azimuth);
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "wadu: %.0f _ wadf: %.0f _ wrdu: %.0f _ wrdf: %.0f", raw_direction, direction, raw_true_direction, true_direction);
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "wavu: %.1f _ wavf: %.1f _ wrvu: %.1f _ wrvf: %.1f", raw_speed, speed, raw_true_speed, true_speed);
        //GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "wind dir: %f --- speed: %f", direction, speed);
        //GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "vx_cor: %f --- vy_cor: %f", vx_cor, vy_cor);
        //GCS_SEND_TEXT(MAV_SEVERITY_INFO, "-----");
    }
}
...
{
    //Zefiro
    if(plane.g2.enable_ballonet > 0) {
        //GCS_SEND_TEXT(MAV_SEVERITY_INFO, "ligado");
        //zefiroControl.operateInflators(&plane.barometer, plane.g2.min_balloon_pressure_diff, plane.g2.max_balloon_pressure_diff, plane.g2.ballonet_deactivated);
        zefiroControl.operateInflators(&plane.barometer, plane.g2.min_balloon_pressure_diff, plane.g2.max_balloon_pressure_diff, 2);
    } else {
        //GCS_SEND_TEXT(MAV_SEVERITY_INFO, "desligado");
