...
AP_InertialNav zef_inertial_nav = AP_InertialNav(ahrs());
...
void ModeHovering::update()
{
    ...
    if (last_msg_ms != gps().last_message_time_ms()) {
        ...
        if( must_reset_loc ) {
            ...
            ahrs().EKF3.getLLH(desired_position);
        }

        // Acquire location
        //const Location &loc = gps().location();
        ahrs().EKF3.getLLH(loc);
    ...
    zefiroControl.update(U_longit_speed, V_lateral_speed, W_vertical_speed,
        ...
        roll, pitch, yaw,
        ret_errors[0], ret_errors[1], ret_errors[2]); //loc.lat, loc.lng, loc.alt);
