...
void
SRV_Channels::tilt_servo(SRV_Channel::Aux_servo_function_t function,
                             uint16_t pwm_value)
{
    if (!function_assigned(function)) {
        return;
    }
    // Iterate through all available servo channels.
    for (uint8_t i = 0; i < NUM_SERVO_CHANNELS; i++) {
        // Get a reference to the current servo channel.
        SRV_Channel &c = channels[i];

        // If the current channel's function matches the desired function,
        // set its output directly to the provided PWM value.
        if (c.function == function) {
            c.set_output_pwm(pwm_value); // Directly set the output PWM
            // Since a servo channel for this function has been found and updated,
            // we can potentially break here if only one channel can map to a function,
            // or continue if multiple channels can share a function (which is less common for servos).
            // For now, assuming only one channel per function is intended per call,
            // but the original code implies iterating through all, so we keep that.
        }
    }
}
/*
